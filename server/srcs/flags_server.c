...
// Imprime l'usage dans le terminal
...
// Parse tout les arguments et les stocks dans la structure
    ...
    while ((opt = getopt(argc, argv, "p:x:y:n:c:t:")) != -1) {
        switch (opt) {
            ...
            case 'n': 
                ...
                break;
            case 'c': config.clients_per_team = atoi(optarg); break;
            case 't': config.time_unit = atoi(optarg); break;
            default: print_server_usage(argv[0]); break;
        }
    }
